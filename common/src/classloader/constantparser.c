...
ConstantPoolInfo* readPoolInfo(const uint8_t** bytes) {
    ...
    switch (tag) {
        ...
        case CONSTANT_STRING:
            ...
            break;
        // These all have the same format. I'll save some byteCode and parse all the same. It's all
        // going into a byte array anyways.
...
ConstantPool* readConstantPool(const uint8_t** bytes) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        switch (cpInfoPtr->tag) {
            case CONSTANT_CLASS: {
                cpInfoPtr->constant->class->name =
                    result->pool[cpInfoPtr->constant->class->nameIndex - 1]->constant->utf8;
                break;
            }
            case CONSTANT_NAME_AND_TYPE: {
                ...
                nameAndTypePtr->descriptor =
                    result->pool[nameAndTypePtr->descriptorIndex - 1]->constant->utf8;
                break;
            }
            case CONSTANT_FIELD_REF:
            case CONSTANT_INTERFACE_METHOD_REF:
            case CONSTANT_METHOD_REF: {
                cpInfoPtr->constant->methodRef->nameAndType =
                    result->pool[cpInfoPtr->constant->methodRef->nameAndTypeIndex - 1]
                        ->constant->nameAndType;
                cpInfoPtr->constant->methodRef->class =
                    result->pool[cpInfoPtr->constant->methodRef->classIndex - 1]->constant->class;
                break;
            }
            case CONSTANT_STRING: {
                cpInfoPtr->constant->string->string =
                    result->pool[cpInfoPtr->constant->string->index - 1]->constant->utf8;
                break;
            }
            default: {
            }
