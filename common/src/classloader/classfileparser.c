...
int initClassFile(const uint8_t* bytes, ClassFile* classPtr) {
    ...
    if (classPtr->superClassIndex == 0) {
        ...
        classPtr->superClass =
            classPtr->constantPool->pool[classPtr->superClassIndex - 1]->constant->class;
    }
...
void getConstantString(char* const buffer, ConstantPoolInfo* item) {
    ...
    switch (item->tag) {
        ...
        case CONSTANT_NAME_AND_TYPE: {
            ...
            sprintf(buffer, "#%d:#%d (%s %s)", constant->nameAndType->nameIndex,
                    constant->nameAndType->descriptorIndex, descriptorString, nameString);
            free(nameString);
        ...
        case CONSTANT_FIELD_REF: {
            ...
            sprintf(buffer, "#%d.#%d (%s %s.%s)", constant->methodRef->classIndex,
                    constant->methodRef->nameAndTypeIndex, descriptor, classString, nameString);
            free(nameString);
            free(classString);
            free(descriptor);
            break;
        ...
        case CONSTANT_METHOD_REF: {
            ...
            sprintf(buffer, "#%d.#%d (%s.%s %s)", constant->methodRef->classIndex,
                    constant->methodRef->nameAndTypeIndex, classString, nameString,
                    descriptorString);
            free(nameString);
            free(classString);
            free(descriptorString);
        ...
        case CONSTANT_STRING:
            ...
            break;
        case CONSTANT_INVOKE_DYNAMIC:
            sprintf(buffer, "#%d:#%d", constant->invokeDynamic->bootstrapMethodAttrIndex,
                    constant->invokeDynamic->nameAndTypeIndex);
            break;
    }
}
            ...
            switch (attribute->type) {
                case ATTRIBUTE_CONSTANT_VALUE: {
                    ConstantPoolInfo* info =
                        constantPool->pool[attribute->info->constantValue->valueIndex - 1];
                    switch (info->tag) {
                        case CONSTANT_FLOAT:
...
void printClassFile(ClassFile* classFilePtr) {
    ...
    uint16_t thisNameIndex = classFilePtr->constantPool->pool[classFilePtr->thisClassIndex - 1]
                                 ->constant->class->nameIndex;
    ...
    uint16_t superNameIndex = classFilePtr->constantPool->pool[classFilePtr->superClassIndex - 1]
                                  ->constant->class->nameIndex;
    ...
    printf("Interface count: %d, Field count: %d, Method count: %d, attributes: %d\n",
           classFilePtr->interfacePool->size, classFilePtr->fieldPool->size,
           classFilePtr->methodPool->size, classFilePtr->attributePool->size);
